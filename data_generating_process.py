...
def create_data_generate_process(mode='mode_1', X=None, T=None, weights=None, x_i_outcome_effect_weight=None, x_i_selection_weight=None):
    ...
    if mode == 'mode_folds_selection_weight_step1':
        # Parameters
        n_samples = 1000  # Number of samples
        d_cov = 10  # Total number of covariates including x_i
    ...
    if mode == 'mode_folds_selection_weight_step2':
        x_i_outcome_effect_weight = 0.3
        treatment_effect = 3
        # Simulate outcomes
        Y0 = np.dot(X[:, 1:], weights) + np.sin(X[:, 1] ** 2)*X[:, 2]**2 + np.sin(X[:, 1] * X[:, 2]) + x_i_outcome_effect_weight * X[:, 0]
